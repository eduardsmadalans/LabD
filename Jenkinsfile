    ...
    stages {
        stage('install-pip-deps') { 
            steps {
				...
				powershell ".\\venv\\Scripts\\python -m pip install -r requirements.txt"
            }
        }
        stage('deploy-to-dev') { 
            steps {
...
def deploy(String env, int port){
	...
	powershell "node_modules/.bin/pm2 delete greetings-app-${env} & EXIT /B 0"
	powershell "node_modules/.bin/pm2 start app.py -n greetings-app-${env} --interpreter .\\venv\\Scripts\\python -- -- ${port}"
}
